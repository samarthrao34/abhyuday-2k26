    ...
    <div className="fixed inset-0 overflow-hidden pointer-events-none select-none z-0">
      ...
      {[...Array(80)].map((_, i) => (
        <motion.div
          ...
          animate={{ opacity: [Math.random() * 0.05, i % 5 === 0 ? 0.6 : 0.35, Math.random() * 0.05], scale: [1, i % 7 === 0 ? 1.8 : 1.3, 1] }}
          transition={{ duration: Math.random() * 5 + 3, repeat: Infinity, ease: 'easeInOut' }}
          style={{ width: `${Math.random() * (i % 10 === 0 ? 4 : 2.5) + 0.5}px`, height: `${Math.random() * (i % 10 === 0 ? 4 : 2.5) + 0.5}px` }}
        />
      ))}
